...
class KOHLER_API:
    ...
    async def b2c_login(self):
        ...
        try:
            ...
            if resp.status != 200:
                ...
                _LOGGER.error("B2C initial request failed with status %s: %s", 
            ...
            if not match:
                ...
                raise KohlerB2CError("Could not extract StateProperties from B2C response")
            ...
            if resp.status != 200:
                resp_text = await resp.text()
                _LOGGER.error("B2C login post failed with status %s: %s", 
                             resp.status, resp_text[:500])
            ...
            if resp.status not in [302, 303]:
                resp_text = await resp.text()
                _LOGGER.error("B2C confirmation failed with status %s: %s", 
                             resp.status, resp_text[:500])
            ...
            if not matches:
                _LOGGER.error("Could not extract code from Location: %s", 
            ...
            if resp.status != 200:
                resp_text = await resp.text()
                _LOGGER.error("Failed to get OAuth tokens: HTTP %s: %s",
                             resp.status, resp_text[:500])
            ...
            try:
                ...
                resp_text = await resp.text()
                _LOGGER.error("Failed to parse OAuth token response as JSON: %s. Response: %s", 
                             e, resp_text[:500])
            ...
            if 'access_token' not in data or 'expires_in' not in data:
                ...
                await self._session.close()
                raise KohlerB2CError("access_token or expires_in missing from OAuth response")

            self._token = data['access_token']
            self._token_expiration = datetime.now() + timedelta(seconds=data['expires_in'])
            self._refresh_token = data.get('refresh_token')
            self._refresh_token_expiration = datetime.now() + timedelta(seconds=data.get('refresh_token_expires_in', 0))
    ...
    async def get_phyn_token(self):
        ...
        try:
            ...
            if resp.status != 200:
                resp_text = await resp.text()
                _LOGGER.error("Failed to get Phyn settings: HTTP %s: %s",
                             resp.status, resp_text[:500])
                raise KohlerTokenError(f"Failed to get Phyn settings: HTTP {resp.status}")
            
            try:
                mobile_data = await resp.json()
            except json.JSONDecodeError as e:
                resp_text = await resp.text()
                _LOGGER.error("Failed to parse Phyn settings response as JSON: %s. Response: %s", 
                             e, resp_text[:500])
            ...
            if "pws_api" not in mobile_data or "app_api_key" not in mobile_data.get("pws_api", {}):
                _LOGGER.error("pws_api or app_api_key missing from Phyn settings response")
                await self._session.close()
            ...
            if resp.status != 200:
                resp_text = await resp.text()
                _LOGGER.error("Failed to get Phyn token: HTTP %s: %s",
                             resp.status, resp_text[:500])
            ...
            try:
                ...
                resp_text = await resp.text()
                _LOGGER.error("Failed to parse Phyn token response as JSON: %s. Response: %s", 
                             e, resp_text[:500])
    ...
    async def token_to_password(self, token):
        ...
        try:
            if 'partner' not in self._mobile_data or 'comm_id' not in self._mobile_data.get('partner', {}):
