...
async def test_b2c_login_missing_state_properties():
    ...
    mock_response = create_mock_response(
        status=200, 
        text_data="<html>No state properties here</html>"
    )
    api._session.get = AsyncMock(return_value=mock_response)
...
async def test_b2c_login_missing_csrf_token():
    ...
    mock_response = create_mock_response(
        status=200,
        text_data='"StateProperties=ABC123"'
    )
    api._session.get = AsyncMock(return_value=mock_response)
...
async def test_b2c_login_missing_auth_code():
    ...
    # Setup mock responses
    initial_response = create_mock_response(status=200, text_data='"StateProperties=ABC123"')
    login_post_response = create_mock_response(status=200)
    redirect_response = create_mock_response(
        status=302, 
        headers={"Location": "https://redirect.com?error=something"}  # No code parameter
    )
...
async def test_b2c_login_invalid_json_in_token_response():
    ...
    responses = [
        ...
        create_mock_response(
            status=302,
            headers={"Location": "https://redirect.com?code=AUTH_CODE"}
        ),
...
async def test_b2c_login_missing_client_info():
    ...
    responses = [
        ...
        create_mock_response(
            status=200,
            json_data={"access_token": "token123", "expires_in": 3600}  # Missing client_info
        ),
    ]
    api._session.get = AsyncMock(side_effect=responses[::2])
    api._session.post = AsyncMock(side_effect=responses[1::2])
...
async def test_get_phyn_token_error_msg_in_response():
    ...
    mock_response = create_mock_response(
        status=200,
        json_data={"error_msg": "Invalid partner credentials"}
    )
    api._session.get = AsyncMock(return_value=mock_response)
...
async def test_get_phyn_token_missing_cognito():
    ...
    mock_response = create_mock_response(
        status=200,
        json_data={"pws_api": {"app_api_key": "key123"}}  # Missing cognito
    )
    api._session.get = AsyncMock(return_value=mock_response)
...
async def test_get_phyn_token_missing_token_field():
    ...
    responses = [
        ...
        create_mock_response(
            status=200,
            json_data={"other_field": "value"}  # Missing token
        ),
    ]
...
async def test_token_to_password_invalid_token():
    ...
    assert "decode" in str(exc_info.value).lower() or "decrypt" in str(exc_info.value).lower()
...
async def test_token_to_password_missing_partner_data():
    ...
    assert "partner" in str(exc_info.value).lower() or "comm_id" in str(exc_info.value).lower()
